{{-- File: resources/views/admin/crops/edit.blade.php --}}
@extends('layouts.app')
...
<div class="page-wrapper" style="max-width:680px;">
    ...
    <div class="page-header">
        <div>
            <h1>✏️ Edit Crop — {{ $crop->name }}</h1>
            <p>Update crop details and availability</p>
        </div>
        ...
        <form action="{{ route('admin.crops.update', $crop) }}" method="POST">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="local_name" class="form-control" value="{{ old('local_name', $crop->local_name) }}">
                </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:1rem;">
                <div class="form-group">
                    ...
                    <select name="growing_season" class="form-control">
                        ...
                        <option value="{{ $s }}" {{ old('growing_season', $crop->growing_season) == $s ? 'selected' : '' }}>{{ $s }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label class="form-label">Growth Days</label>
                    <input type="number" name="growth_days" class="form-control" value="{{ old('growth_days', $crop->growth_days) }}" min="1">
                </div>
                <div class="form-group">
                    ...
                    <select name="water_requirement" class="form-control">
                        ...
                        <option value="{{ $w }}" {{ old('water_requirement', $crop->water_requirement) == $w ? 'selected' : '' }}>{{ $w }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group">
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
                    ...
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $crop->is_active) ? 'checked' : '' }}
            ...
            <div style="display:flex;justify-content:flex-end;gap:0.75rem;">
                ...
                <button type="submit" class="btn btn-primary">
                    <i data-lucide="check"></i> Update Crop
                </button>
            </div>
        </form>
    </div>
